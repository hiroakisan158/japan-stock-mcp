...
まず試したいセクターだけ取得して動作確認し、その後全銘柄に広げるのが現実的です。
...
```bash
# 株価（yfinance）— 全銘柄で 30〜60 分
make prices

# 四半期財務（J-Quants）— 全銘柄で約 40 分
make quarters
```
...
```bash
make prices-sector SECTOR=電気機器
make quarters-sector SECTOR=電気機器
```
...
初回セットアップ完了後は `make sync` / `make sync-sector` を使います。EDINET 差分 → 株価 → 四半期 の3ステップを自動で順番に実行します。

> **注意**: `make sync` は差分更新のみです。新しいセクターを追加したい場合は先に `make batch-init-sector` で初期ロードしてください。
